 ...
 * QuizList Model
 */
class QuizModelResponses extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		/*$query->select('u.id, u.content, u.valeur, u.id_question, a.content');
        $query->from('#__reponse AS u');
        $query->leftJoin('#__question AS a ON a.id = u.id_question');*/
			// Check for a search filter.
           if ($this->getState('filter.search'))
            {
               $query->where('( ' . $db->quoteName('content') . ' LIKE \'%' . $db->escape($this->getState('filter.search')) . '%\' )');
          }
		...
		protected function populateState($ordering = null, $direction = null)
     {
        ...
        $search = $this->getUserStateFromRequest($this->context . '.filter.search', 'filter_search');
         ...
         // List state information.
          parent::populateState('content', 'asc');
      }
}
